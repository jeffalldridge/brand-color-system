  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col">
        {/* Name Row */}
        <div
          className={`px-1.5 pt-1.5 pb-1 border-b ${bgIsLight ? "border-black/5" : "border-white/5"}`}
        >
          <input
            type="text"
            value={color.name}
            ...
            className={`w-full text-sm font-semibold px-1 py-1 rounded border-transparent focus:outline-none transition-colors ${
              bgIsLight
                ? "text-black focus:bg-black/5"
        ...
        <div className="px-2 py-1.5 flex gap-1 items-center justify-between">
          <input
            ...
            className={`min-w-0 flex-1 text-[10px] font-mono px-1 py-1 rounded border border-transparent focus:outline-none transition-colors uppercase ${
              bgIsLight
                ? "text-black/80 hover:bg-black/5 focus:bg-black/5"
        ...
        {hasAdjustments && (
          <div
            className={`px-2 text-[9px] font-mono flex items-center justify-between pb-1 mt-[-2px] ${txtVal}`}
          >
            <span className="opacity-60">adj:</span>
            <div className="flex items-center gap-1.5">
              <div
                className={`w-2 h-2 rounded-full border ${bgIsLight ? "border-black/20" : "border-white/20"}`}
                style={{ backgroundColor: color.hex }}
                title={`Source: ${color.hex}`}
              />
              ...
              />
              <span
                className={`font-semibold tracking-wider ${bgIsLight ? "text-black/80" : "text-white/80"}`}
              >
        ...
        {showAdjustmentsLocal && (
          <div
            className="flex flex-col gap-2 relative mt-1 px-2 pb-2"
            style={sliderVars}
          >
            {(() => {
              ...
              const absH =
                Math.round(
                  ((((bH + color.hueShift) % 360) + 360) % 360) * 10000,
                ) / 10000;
              const absC =
                Math.round(
                  (bC > 0.005
                    ? Math.max(0, bC * (1 + color.saturationShift / 100))
                    : Math.max(0, (color.saturationShift / 100) * 0.15)) *
                    10000,
                ) / 10000;
              const absL =
                Math.round(
                  Math.max(0, Math.min(1, bL + color.lightnessShift)) * 10000,
                ) / 10000;
              ...
              const sliders = [
                {
                  ...
                  delta:
                    color.hueShift !== 0
                      ? `${color.hueShift > 0 ? "+" : ""}${color.hueShift}`
                      : null,
                  min: 0,
                  max: 360,
                  step: 1,
                  ...
                  onChange: (v: number) => {
                    ...
                    onAdjustmentsChange(index, {
                      hueShift: Math.round(normalized),
                    });
                  },
                },
                {
                  ...
                  delta:
                    color.saturationShift !== 0
                      ? `${color.saturationShift > 0 ? "+" : ""}${color.saturationShift}%`
                      : null,
                  min: 0,
                  max: chromaMax,
                  step: 0.001,
                  ...
                  onChange: (v: number) => {
                    if (bC > 0.005) {
                      const s = (v / bC - 1) * 100;
                      onAdjustmentsChange(index, {
                        saturationShift: Math.round(
                          Math.max(-100, Math.min(100, s)),
                        ),
                      });
                    } else {
                      const s = (v / 0.15) * 100;
                ...
                },
                {
                  ...
                  delta:
                    color.lightnessShift !== 0
                      ? `${color.lightnessShift > 0 ? "+" : ""}${color.lightnessShift.toFixed(2)}`
                      : null,
                  min: 0,
                  max: 1,
                  step: 0.01,
                  ...
                  onChange: (v: number) => {
                    const shift = v - bL;
                    onAdjustmentsChange(index, {
                      lightnessShift: parseFloat(shift.toFixed(2)),
                    });
                  },
                },
              ];
              ...
              return sliders.map((s) => (
                <div key={s.label} className="group/slider">
                  <div className="flex justify-between items-baseline opacity-70 group-hover/slider:opacity-100 transition-opacity">
                    <label
                      className={`text-[9px] ${txtMuted} uppercase tracking-wider font-medium`}
                    >
                      {s.label}
                    </label>
                    <div className="flex items-baseline gap-1">
                      <span
                        className={`text-[9px] font-mono font-semibold ${bgIsLight ? "text-black/80" : "text-white/80"}`}
                      >
                        {s.absValue}
                      </span>
                      {s.delta && (
                        <span className={`text-[8px] font-mono ${txtMuted}`}>
                          {s.delta}
                        </span>
                      )}
                    </div>
