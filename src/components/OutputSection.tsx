...
export default function OutputSection({ families, bgIsLight }: OutputSectionProps) {
  ...
  const btnBorder = bgIsLight ? 'border-black/25 text-black/70 hover:text-black hover:bg-black/10' : 'border-white/30 text-white/80 hover:text-white hover:bg-white/15';
  const hoverRow = bgIsLight ? 'hover:text-black hover:bg-black/5' : 'hover:text-white hover:bg-white/10';
        ...
        {families.map((family) => {
          ...
          return (
            <div key={family.brand.id}>
              <button
                onClick={() => setExpandedFamily(isExpanded ? null : family.brand.id)}
              ...
              >
                ...
                />
                <span className={`text-xs font-medium ${txt}`}>{family.brand.name}</span>
                <span className={`text-[10px] font-mono ${txtMuted}`}>{family.adjustedHex}</span>
              ...
              {isExpanded && (
                <div className="ml-5 mb-3 overflow-x-auto">
                  <table className="w-full text-[10px] font-mono border-collapse">
                    <thead>
                      <tr className={`${txtMuted} border-b ${bgIsLight ? 'border-black/5' : 'border-white/5'}`}>
                        <th className="text-left font-semibold py-1.5 pr-3">Step</th>
                        <th className="text-left font-semibold py-1.5 pr-3">Hex</th>
                        <th className="text-left font-semibold py-1.5 pr-3">RGB</th>
                        <th className="text-left font-semibold py-1.5 pr-3">HSL</th>
                        <th className="text-left font-semibold py-1.5 pr-3">OKLCH</th>
                        <th className="text-left font-semibold py-1.5">Gamut</th>
                      </tr>
                    </thead>
                    <tbody>
                      {family.shades.map((shade) => (
                        <tr key={shade.step} className={`${txtRow} ${hoverRow} border-b border-transparent ${bgIsLight ? 'hover:border-black/5' : 'hover:border-white/5'}`}>
                          <td className="py-1 pr-3 font-bold">{shade.step}</td>
                          <td
                            className={`py-1 pr-3 cursor-pointer ${hoverCell}`}
                            onClick={() => copyToClipboard(shade.hex, `${family.brand.name}-${shade.step}-hex`)}
                          >
                            {shade.hex}
                            {copiedKey === `${family.brand.name}-${shade.step}-hex` && (
                              <span className="ml-1 text-green-500">ok</span>
                            )}
                          </td>
                          <td
                            className={`py-1 pr-3 cursor-pointer ${hoverCell}`}
                            onClick={() => copyToClipboard(shade.rgb, `${family.brand.name}-${shade.step}-rgb`)}
                          >
                            {shade.rgb}
                            {copiedKey === `${family.brand.name}-${shade.step}-rgb` && (
                              <span className="ml-1 text-green-500">ok</span>
                            )}
                          </td>
                          <td
                            className={`py-1 pr-3 cursor-pointer ${hoverCell}`}
                            onClick={() => copyToClipboard(shade.hsl, `${family.brand.name}-${shade.step}-hsl`)}
                          >
                            {shade.hsl}
                            {copiedKey === `${family.brand.name}-${shade.step}-hsl` && (
                              <span className="ml-1 text-green-500">ok</span>
                            )}
                          </td>
                          <td
                            className={`py-1 pr-3 cursor-pointer ${hoverCell}`}
                            onClick={() => copyToClipboard(shade.oklchCss, `${family.brand.name}-${shade.step}-oklch`)}
                          >
                            {shade.oklchCss}
                            {copiedKey === `${family.brand.name}-${shade.step}-oklch` && (
                              <span className="ml-1 text-green-500">ok</span>
                            )}
                          </td>
                          <td className="py-1">
...
function HowItWorks({ bgIsLight }: { bgIsLight: boolean }) {
  ...
  const linkColor = bgIsLight ? 'text-blue-600 hover:text-blue-800' : 'text-blue-400 hover:text-blue-300';
  ...
  return (
    <div className={`border-t pt-3 ${border}`}>
      ...
      {open && (
        <div className={`mt-3 space-y-4 text-[11px] leading-relaxed ${txt} max-w-3xl`}>

          <Section title="Why OKLCH?" headingColor={headingColor}>
            In HSL, a &ldquo;50% lightness&rdquo; yellow looks noticeably brighter than
            a &ldquo;50% lightness&rdquo; blue. The numbers match, but your eyes
            disagree. <Link href="https://oklch.com" className={linkColor}>OKLCH</Link> is
            a perceptually uniform color space where equal values produce equal visual
            weight. <Code bg={codeBg}>L</Code> is lightness (0 to 1), <Code bg={codeBg}>C</Code> is
            chroma (vividness), <Code bg={codeBg}>H</Code> is hue (0 to 360°). Every swatch in the
            same column shares identical perceived brightness, regardless of hue. Colors outside
            your screen&apos;s gamut are{' '}
            <Link href="https://www.w3.org/TR/css-color-4/#gamut-mapping" className={linkColor}>mapped back</Link>{' '}
          ...
          <Section title="Adjustments" headingColor={headingColor}>
            Per-color sliders fine-tune without altering the original
            hex. <strong>H</strong> rotates the hue (nudge a blue toward teal,
            for instance). <strong>C</strong> scales chroma for more vivid or more
            muted shades. <strong>L</strong> shifts base lightness before the ramp is
            applied. The tick mark on each slider indicates the original value.
          </Section>

          <Section title="Exports" headingColor={headingColor}>
            <strong>CSS</strong> copies hex custom properties for any stylesheet.{' '}
            <strong>Tailwind</strong> outputs an{' '}
            <Link href="https://tailwindcss.com/docs/theme" className={linkColor}>@theme</Link>{' '}
            block with OKLCH values for v4.{' '}
            <strong>Tokens</strong> downloads{' '}
            <Link href="https://tr.designtokens.org/format/" className={linkColor}>W3C Design Tokens</Link>{' '}
...
function Section({ title, headingColor, children }: { title: string; headingColor: string; children: React.ReactNode }) {
  return (
    <div>
      <h3 className={`text-[10px] uppercase tracking-widest font-semibold mb-1 ${headingColor}`}>{title}</h3>
      <p>{children}</p>
    </div>
  );
}
...
function Link({ href, className, children }: { href: string; className: string; children: React.ReactNode }) {
  return <a href={href} target="_blank" rel="noopener noreferrer" className={`underline underline-offset-2 ${className}`}>{children}</a>;
}

function Code({ bg, children }: { bg: string; children: React.ReactNode }) {
  return <code className={`${bg} px-1 py-0.5 rounded text-[10px] font-mono`}>{children}</code>;
}
