...
export default function BrandColorEditor({ brandColors, families, bgIsLight, onColorChange, onNameChange, onAdjustmentsChange, onReorder, onAddColor, onRemoveColor, onSetHero, onToggleVisibility }: BrandColorEditorProps) {
  ...
  const familyMap = useMemo(() => new Map(families.map(f => [f.brand.id, f])), [families]);
      ...
      >
        <div className="grid gap-3 pb-4" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))' }}>
          {brandColors.map((color, index) => {
            ...
            const hasAdjustments = color.hueShift !== 0 || color.saturationShift !== 0 || color.lightnessShift !== 0;
          ...
          <div className="min-w-0 flex flex-col">
            <button
              onClick={onAddColor}
              className={`w-full rounded-xl border-2 border-dashed flex flex-col items-center justify-center gap-2 min-h-[110px] h-full transition-colors ${bgIsLight
            ...
            >
              ...
              <span className="text-[10px] uppercase tracking-wider font-medium">Add Color</span>
            </button>
          </div>
        </div>
      </SortableContext>
