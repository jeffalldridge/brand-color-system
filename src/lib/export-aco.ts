...
export function generateAcoFile(families: ShadeFamily[]): ArrayBuffer {
  ...
  for (const e of entries) {
    ...
    offset += 2; // color space: 0 = RGB
    view.setUint16(offset, e.r, false);
    offset += 2;
    view.setUint16(offset, e.g, false);
    offset += 2;
    view.setUint16(offset, e.b, false);
    offset += 2;
    view.setUint16(offset, 0, false);
    offset += 2; // padding (z)
  }
  ...
  for (const e of entries) {
    ...
    view.setUint16(offset, 0, false);
    offset += 2; // padding
    view.setUint16(offset, e.name.length + 1, false);
    offset += 2; // name length (incl. null)
    for (let i = 0; i < e.name.length; i++) {
      view.setUint16(offset, e.name.charCodeAt(i), false);
      offset += 2;
    }
    view.setUint16(offset, 0, false);
